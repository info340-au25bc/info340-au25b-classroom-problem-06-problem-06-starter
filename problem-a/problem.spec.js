...
describe('Manipulates the DOM', () => {
  ...
  test('Implemented the renderPaletteRow() function', () => {
    ...
    expect(colorBoxes.eq(0).css('background-color')).toMatch(/red|rgb\(255, 0, 0\)/); //colors should be right (processed as rgb)
    expect(colorBoxes.eq(1).css('background-color')).toMatch(/green|rgb\(0, 128, 0\)/);
    expect(colorBoxes.eq(2).css('background-color')).toMatch(/blue|rgb\(0, 0, 255\)/);    
  })
